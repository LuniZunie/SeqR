            ...
            while (lines != 'done') {
              ...
              if (lines != 'done') {
                for (const [ line, s, e ] of lines) {
                  ...
                  settings.draw.r[group_i] = { s: Math.min(s, settings.draw.r[group_i].s), e: Math.max(e, settings.draw.r[group_i].e) };
      ...
      await new Promise(r_read => {
        ...
        Lines(chunkLocs, null, null, range.s, range.e, data._ks()).then(async gen => {
          ...
          while (lines != 'done') {
            ...
            if (lines != 'done') {
              for (const [ line, s, e, c, w, x ] of lines) {
                ...
                const drawLine = async function(s, e) {
...
async function Lines(file, type, strand, s, e, lines) {
  ...
  const check = new RegExp(`[^${lines?.join('')}]( \\d+){1,} (\\d+\\n|\\d+$)`, 'g');
